...
**2026-07-08 更新(已用重現矩陣實測,不是斷言)**:B 類有一個被忽略的子型——
**「內容過濾剝掉內嵌 script」或「政策注入 CSP `script-src 'self'`」**——會讓瀏覽器收到一份
「看似沒有 JavaScript」的 index.html(磁碟檔其實完好!),這正是現場「index.html 沒有 JS」誤判
的來源。**此子型已在程式端修掉**:兩元件 JS 抽成外部同源 `main.js`,內容過濾/CSP 都放行外部
script。重現/迴歸工具:`python verify/repro_component_banner.py`(印紅/綠矩陣,#3 剝內嵌、
#4 CSP 已恆綠;#1 全擋、#5 拖慢仍恆紅=環境問題)。console 簽名鑑別:`fetch error`/`net::ERR`
= 請求被擋(A/全擋);只有 `timeout error` = 到得了但 ready 沒回來(拖慢或舊瀏覽器)。
