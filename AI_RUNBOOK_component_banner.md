...
**2026-07-08 更新(誠實分級,別重蹈「把建模當現場定論」的覆轍)**:
- **現場最對得上的是 A/#1**:proxy 在非 localhost 路徑整條擋 `/component/`(真 forward proxy 重現 →
  console `source error 4xx`)。**元件碼修不了**(iframe HTML 被擋)。對症:`app.py` 啟動偵測「非 localhost 開」
  → 主頁顯示指示;一律用 `http://localhost` 開。
- **B 類一個『推定、現場未證實』的子型**:內容過濾剝掉內嵌 script,或 CSP `script-src 'self'` 禁內嵌 →
  瀏覽器收到「看似沒 JavaScript」的 index.html(磁碟檔其實完好)。這**只是我方用真 proxy『建模』**出來、
  能解釋現場「index.html 沒 JS」報告的**可能**成因之一(該報告也可能是看錯檔/搬壞檔)。已在程式端加固:
  兩元件 JS 抽成外部同源 `main.js`,內容過濾/CSP 皆放行外部 script。重現/迴歸(真 forward proxy):
  `python verify/repro_real_proxy.py`(strip 情境內嵌版紅、外部版綠;block+IP 情境元件仍被擋但 app guard 出現);
  較早的 page.route 版是 `verify/repro_component_banner.py`(忠實度較低)。
- **console 簽名鑑別**:`source/fetch error`/`net::ERR`/4xx = 請求被擋(A/#1);只有 `timeout error` = 到得了但
  ready 沒回來(剝內嵌/CSP/拖慢/舊瀏覽器)。
- **另一種現場已證同症**:搬運把 `main.js` 弄丟或改名(Gmail-safe 打包 `.js→.js.txt`)→ 404 → 同橫幅;
  `app.py` 啟動會檢查兩個 `main.js` 存在與否並在主頁明講。
