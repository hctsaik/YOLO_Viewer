...
# 🛟 安全模式下不顯示本提示:它的前提是「元件載入失敗」,但安全模式根本不掛元件、不抓 /component/,
# 這條提示就成了純噪音(還會誤導使用者去改 proxy)。註:此處在 `ss` 別名定義之前,直接用 st.session_state。
_safe_now = st.session_state.get("safe_mode", default_safe_mode())
if not _is_localhost and not _safe_now:
    st.info(
...
def _pick_model_file():
    """開原生檔案總管挑模型權重(.pth)。同 _pick_folder:headless/無顯示則回 None、不崩。"""
    try:
        import tkinter as tk
        from tkinter import filedialog
        root = tk.Tk()
        ...
        path = filedialog.askopenfilename(
            parent=root, title="選擇 DINOv2 權重檔",
            filetypes=[("模型權重", "*.pth *.pt"), ("所有檔案", "*.*")])
        root.destroy()
        return path or None
    except Exception:
        return None
...
with st.sidebar:
    # ── 側欄版面順序(User 回饋 2026-07-13):資料來源 → 安全模式 → CV 工具箱 ──────────────
    # 陷阱:「資料來源」的 📁 瀏覽鈕會 st.rerun()(見 _folder_field),而安全模式那組是 keyed widget。
    # 若照視覺順序把安全模式寫在瀏覽鈕『之後』,按一次瀏覽 → 腳本在 rerun 處中斷 → 安全模式的
    # toggle/滑桿這輪從未實例化 → 被 Streamlit 當孤兒清成預設(§4.l 鐵律,本專案已踩過 ≥3 次)。
    # 解法:先用 container 把兩個視覺插槽依「使用者要的順序」佔好,再以「§4.l 要的順序」填內容
    #   —— container 的位置在建立時就定了,填的先後不影響它顯示在哪。兩個約束因此同時成立。
    _slot_source = st.container()   # 視覺第 1 格:📁 資料來源
    _slot_safe = st.container()     # 視覺第 2 格:🛟 安全模式
    #                                 視覺第 3 格:🧰 CV 工具箱(st.sidebar.expander,見下方 ss.idx 定案後)

with _slot_safe:
    # ── 🛟 安全模式(2026-07-10)──────────────────────────────────────────────────────────
    # 現場某些機器(內容過濾 proxy／防毒擋掉 /component/ 資產,或端點防護在每頁注入腳本擋住
    # iframe 內的元件腳本)會讓 viewer/thumbwall 兩個「Streamlit 自訂元件」永遠送不出
    # componentReady → 60 秒後跳「trouble loading the component」橫幅,整個工作台不能用。
    # 這兩類故障都卡在『自訂元件 iframe』這條路上,但主頁的原生 st.* 元件是好的(頁面本身會算繪)。
    # 故提供完全不走 iframe、不抓 /component/ 的降級檢視:縮圖牆 → st.image + st.button;
    # 主 viewer → overlay.draw 在 server 端燒框後 st.image;縮放/平移 → viewport.crop_rect 裁切。
    # ★ widget 順序鐵律(§4.l):這些 keyed widget 必須在任何會呼叫 st.rerun() 的按鈕『之前』
    #   實例化(📁 瀏覽鈕、安全模式縮圖鈕、導覽鈕),否則會被 Streamlit 當孤兒 widget 清成預設值。
    #   → 因此整組『在程式碼裡』仍是 sidebar 第一段,只是被填進視覺第 2 格(_slot_safe)。
    # 2026-07-13(User 裁決):完整模式(OpenSeadragon)是預設,安全模式改為明確 opt-in ——
    #   run_safe.bat / CVR_SAFE_MODE=1 / 本開關。理由見 runtime_mode.default_safe_mode。
    ss.setdefault("safe_mode", default_safe_mode())
    ss.setdefault("safe_zoom", 1.0)
    ss.setdefault("safe_cx", 50)
    ss.setdefault("safe_cy", 50)
    with st.expander("🛟 安全模式(元件載不出來時)", expanded=bool(ss.get("safe_mode"))):
        st.toggle("啟用安全模式(不使用自訂元件)", key="safe_mode",
                  help="影像檢視器／縮圖牆出現「trouble loading the component」橫幅時開啟。"
                       "改用純 server-side 算繪,瀏覽器不需載入 /component/ 資產、也不需執行 iframe 內腳本。")
        if ss.get("safe_mode"):
            st.slider("🔍 放大倍率", 1.0, 16.0, key="safe_zoom", step=0.5)
            st.slider("↔ 水平位置 %", 0, 100, key="safe_cx")
            st.slider("↕ 垂直位置 %", 0, 100, key="safe_cy")
            st.caption("安全模式沒有滾輪縮放／拖曳平移／框選 ROI／點擊取像素值;導覽用上方"
                       "「⟵ 上一張／下一張 ⟶」。要這些互動請關掉本開關(或用 run_full.bat)。")

# 視覺第 1 格,但程式碼順序在安全模式『之後』—— 因為 _folder_field 的 📁 瀏覽鈕會 st.rerun()。
with _slot_source:
    st.header("📁 資料來源")
...
_CMP_VIEW_MODES = [("pixel", "🖼️ 像素疊合"), ("box", "📦 偵測框疊合"), ("dino", "🔥 DINO 語意差異")]
_CMP_PIXEL_MODES = [("side", "並排"), ("diff", "差異"), ("blend", "混合")]
# _cmp_filter 已上移為模組層級共用函式(見「信心雙界過濾」節),供本節與單張模式 kept 共用。

# ── 🔥 DINO 語意差異(設計 27_dinodiff.md)────────────────────────────────────────────
# 權重找法(dinodiff.resolve_model_path,永不上網):📁 選的 → CVR_DINO_MODEL → 掃這些資料夾。
# CVR_MODELS_DIR 讓 Tauri 殼日後把它的共用模型夾接進來,不需要改任何程式。
_MODEL_DIRS = ([Path(os.environ["CVR_MODELS_DIR"])] if os.environ.get("CVR_MODELS_DIR") else []) \
              + [ROOT / "models"]
_DINO_HUB = ROOT / "5_PG_Develop" / "dinov2_hub"


@st.cache_resource(show_spinner=False)
def _dino_extractor(model_path):
    """模型物件只載一次(cache_resource:不可序列化的重物件)。換權重路徑 → 自然是新的 cache key。"""
    return dinodiff.Dinov2PatchExtractor(model_path, _DINO_HUB)


@st.cache_data(show_spinner=False)
def _dino_feats(img_path, model_path, _mtime):
    """patch 特徵快取。CPU 前向約 1 秒/張 —— 沒有這層,調一格 alpha 滑桿就要重跑模型(設計 §3)。
    _mtime 進 cache key:影像被換掉(同路徑不同內容)時特徵要跟著失效。"""
    return _dino_extractor(model_path)(_display_rgb(img_path))


def _render_dino_diff(it_a, it_b, arr_a, slot_model):
    """🔥 DINO 語意差異(設計 27_dinodiff.md §3 管線)。回 P1 探針 dict。

    ★ widget 順序鐵律(§4.l):模型列的『📁 選檔』鈕會 st.rerun(),若排在三個滑桿之前,
      按一次選檔就會把 alpha/K/門檻清成預設 → 呼叫端先用 container 佔好視覺位置(『檢視方式』
      右邊那格),這裡讓滑桿先實例化、模型列後填。視覺順序與程式碼順序因此可以不同。
    """
    cc = st.columns(3)
    alpha = cc[0].slider("熱力圖濃度", 0.0, 1.0, 0.55, 0.05, key="dino_alpha",
                         help="0 = 只看原圖;1 = 只看熱力圖。冷區永遠維持原圖不被蒙色。")
    topk = cc[1].slider("框出前 K 大差異區", 0, 10, 3, key="dino_topk")
    thr = cc[2].slider("熱區門檻", 0.1, 0.95, 0.5, 0.05, key="dino_thr",
                       help="熱力圖 ≥ 此值才算一塊『差異區』。調高 = 只框最紅的。")

    model_path = dinodiff.resolve_model_path(ss.get("dino_model_path"), os.environ, _MODEL_DIRS)
    with slot_model:
        mc = st.columns([5, 1], vertical_alignment="bottom")
        mc[0].markdown("🧠 **DINOv2 權重**")
        mc[0].caption(f"`{model_path.name}`" if model_path else "**尚未選擇**(需要 .pth)")
        if mc[1].button("📁", key="dino_pick_model", help="選擇 DINOv2 權重檔(.pth)"):
            picked = _pick_model_file()
            if picked:
                ss["dino_model_path"] = picked
                st.rerun()

    if model_path is None:
        # 離線鐵則:不下載、不報錯,只告訴使用者怎麼把權重放好(設計 §4)。
        st.warning(
            "找不到 DINOv2 權重(`.pth`)。三種方式擇一:①按右上 **📁** 選檔;"
            f"②把權重放進 `{ROOT / 'models'}`(檔名如 `dinov2_vits14.pth`);"
            "③設環境變數 `CVR_DINO_MODEL` 指向權重檔。**本功能永遠不會上網下載。**")
        return {"model": 0, "score": "", "regions": ""}

    try:
        with st.spinner("DINOv2 特徵抽取中(首次載入模型約需數秒)…"):
            fa = _dino_feats(it_a["path"], str(model_path), os.path.getmtime(it_a["path"]))
            fb = _dino_feats(it_b["path"], str(model_path), os.path.getmtime(it_b["path"]))
    except ImportError:                          # torch 是選用相依(requirements.txt):沒裝就好好說
        st.error("這台機器沒有安裝 **torch**(DINO 檢視需要)。安裝 CPU 版即可:\n\n"
                 "`pip install torch --index-url https://download.pytorch.org/whl/cpu`\n\n"
                 "其餘檢視方式(像素疊合 / 偵測框疊合)不受影響。")
        return {"model": 0, "score": "", "regions": ""}
    except Exception as e:                       # 權重壞掉/架構不符 → 說清楚,不要整頁 traceback
        st.error(f"載入 DINOv2 權重失敗:{e}")
        return {"model": 0, "score": "", "regions": ""}

    h, w = arr_a.shape[0], arr_a.shape[1]
    dmap = dinodiff.cosine_distance_map(fa, fb)
    score = dinodiff.diff_score(dmap)
    heat_g = dinodiff.normalize_heat(dmap)
    out = dinodiff.colorize_overlay(arr_a, dinodiff.upsample(heat_g, w, h), alpha)

    # Top-K 在 grid 尺度上找(37×37),回來的 bbox 已是影像座標。白框 = 與類別色盤不撞色。
    regions = dinodiff.top_regions(heat_g, w, h, k=topk, thr=thr)
    if regions:
        out = overlay.draw(out, [{"bbox": r["bbox"], "cls": "差異", "conf": r["score"]}
                                 for r in regions],
                           color=(255, 255, 255), thickness=2, draw_label=True)

    st.image(out, width=_STRETCH,
             caption=f"🔵 A:{it_a['name']}　🟠 B:{it_b['name']}(熱力圖疊在 A 上)")
    st.caption(
        f"🔥 **語意差異 {score:.1f} / 100** · 熱區 **{len(regions)}** 塊 · 模型 `{model_path.name}` · "
        f"熱力圖真實解析度 {heat_g.shape[0]}×{heat_g.shape[1]}(指出「哪一區」不同,非像素級)")
    if score < 1.0:
        st.info("兩張圖在語意上幾乎相同(分數 < 1)—— 熱力圖刻意維持全冷,不做相對拉伸,"
                "以免把毫無差異的兩張圖畫成一片紅。")
    return {"model": 1, "score": f"{score:.1f}", "regions": str(len(regions))}


def _render_compare():
    """兩圖疊圖比較(設計 23_compare.md §9):在縮圖牆標記的 2 張影像(①=A 藍 / ②=B 橘),
    切換像素疊合(並排/差異/混合,重用 framecompare)、偵測框疊合,或 DINO 語意差異熱力圖
    (27_dinodiff.md)。"""
    ...
    dino_probe = {"model": "", "score": "", "regions": ""}
    ...
    if view_mode == "pixel":
        ...
        # 模型列佔『檢視方式』右邊那格(pixel 檢視是放「疊合方式」的位置)——視覺上跟檢視選擇同一列,
        # 但真正填內容在滑桿實例化之後(§4.l,見 _render_dino_diff docstring)。
        dino_probe = _render_dino_diff(it_a, it_b, arr_a, c0[1].container())
    else:
    ...
    # DINO 三欄(27_dinodiff.md §5.1):非 dino 檢視時為空字串,同 view-mode 的既有語義。
    ss["_cmp_probe"] = {"view_mode": view_mode,
                        "dino_model": dino_probe["model"],
                        "dino_score": dino_probe["score"],
                        "dino_regions": dino_probe["regions"]}
...
# ==================== 🛟 安全模式:不走自訂元件的 server-side 降級算繪 ====================
# 契約對齊(是同語義的替身,不是另一套行為):
#   縮圖牆 → 同一組 _thumb() PNG(同 _cls_color 調色盤)、同 ss.idx 選取語義、同 _toggle_cmp_mark 比較標記。
#   主 viewer → 元件版是「送未燒框的圖 + dets,由 iframe 內 JS 畫向量框」;安全模式改在 server 端用
#              overlay.draw 燒上同色框 + cls·conf 標籤 → 視覺等價,但瀏覽器不必抓 /component/、
#              也不必執行任何 iframe 內腳本(A 類資產被擋、B 類腳本被封 兩種故障都繞開)。
#   縮放/平移 → 純 server 端裁切(viewport.crop_rect,既有 M1 純模組),不需要 OpenSeadragon。
_SAFE_DISP_W, _SAFE_DISP_H = 1100, 720
_SAFE_TW_H = 1100


def _safe_thumbwall(items, sel, show, classes, conf_thr, marks, markable=True, start=0):
    """元件版 thumbwall 的原生替身;選取/比較標記直接改 ss 並 rerun(與元件事件分派等價)。
    分頁後 items 是本頁切片,start=本頁第一張的全域索引;sel 是全域索引。widget key 用全域索引
    (跨頁唯一,避免 Streamlit 以為是同一顆按鈕)。"""
    st.caption("🛟 安全模式縮圖牆")
    # 元件版縮圖牆是 iframe 內的密排網格,620px 就看得到好幾張;安全模式一張要吃
    # 「圖 + 檔名 caption + 選取/🔀 兩顆鈕」約 300px,620px 只塞得下 2 張(User 回饋:
    # 「不夠高、有些縮圖就不見了」)。拉到與中欄 viewer(720)+ 導覽列相當的高度,一屏看 3–4 張。
    with st.container(height=_SAFE_TW_H):
        for i, it in enumerate(items):
            gi = start + i
            png = _thumb(it["path"], dets=it["detections"], show=show,
                         conf_thr=conf_thr, classes=classes)
            st.image(png, width=_STRETCH)
            s = it["sidecar"]
            badge = ("⭐" if s.get("bookmarked") else "") + ("✓" if tagging.is_reviewed(s) else "")
            cmp_i = ("①" if (marks and it["name"] == marks[0])
                     else "②" if (len(marks) > 1 and it["name"] == marks[1]) else "")
            # 檔名放 caption(窄欄可換行、看得到全名);按鈕標籤保持短,否則會被截成 "lot42_fram"。
            st.caption(f"{'▶ ' if gi == sel else ''}{it['name']} {badge}{cmp_i} · {len(it['detections'])} 框")
            c_sel, c_mk = st.columns([2, 1])
            # 目前這張:primary + disabled → 既是「你在這裡」的視覺標示,也擋掉無意義的重選 rerun。
            if c_sel.button("選取", key=f"sm_tw_{gi}", width=_STRETCH,
                            type="primary" if gi == sel else "secondary", disabled=(gi == sel)):
                ss.idx = gi
                st.rerun()
            # 兩顆都 stretch,否則 🔀 會被窄欄的右邊界切掉。
            if markable and c_mk.button("🔀", key=f"sm_mk_{gi}", width=_STRETCH,
                                        help="標記為比較影像 A/B"):
                _toggle_cmp_mark(it["name"])
                st.rerun()


def _safe_viewer(cur, kept, rois_draw, meta, focus_bbox):
    """元件版 osd_viewer 的原生替身:燒框 →(可選)server 端裁切縮放 → st.image。"""
    rgb = _display_rgb(cur["path"])
    if _adjustments_active():
        rgb = _apply_adjustments(rgb)
    # 逐類別色分組畫(與縮圖/元件版同 _cls_color;overlay.draw 每次回新陣列,可鏈式)。
    drawn = rgb
    by_color = {}
    for d in kept:
        by_color.setdefault(_cls_color(d.get("cls")), []).append(d)
    for col, group in by_color.items():
        drawn = overlay.draw(drawn, group, color=col, thickness=2, draw_label=True)
    if rois_draw:
        drawn = overlay.draw(
            drawn,
            [{"bbox": r["bbox"], "cls": r.get("label", "ROI"), "conf": 1.0} for r in rois_draw],
            color=(255, 255, 0), thickness=2, draw_label=True)

    src_h, src_w = int(drawn.shape[0]), int(drawn.shape[1])
    fit = viewport.fit_zoom(src_w, src_h, _SAFE_DISP_W, _SAFE_DISP_H)
    factor = float(ss.get("safe_zoom", 1.0))
    # 🎯 Focus Object 三態(對齊元件版語義):None=模式關閉;[]=模式開但本圖無框 → 兩者都不聚焦,
    #    回到側欄倍率/位置(倍率預設 1.0 = 整張,不殘留上一張的聚焦);非空=聚焦該框(中心對準,
    #    倍率取『框約填滿顯示框』與側欄倍率的較大者;上限 32× 防止極小框把裁切算爆)。
    if focus_bbox:
        cx = focus_bbox[0] + focus_bbox[2] / 2.0
        cy = focus_bbox[1] + focus_bbox[3] / 2.0
        bw, bh = max(1, int(focus_bbox[2])), max(1, int(focus_bbox[3]))
        fill = min(_SAFE_DISP_W / (bw * fit * 1.6), _SAFE_DISP_H / (bh * fit * 1.6))
        factor = max(factor, min(32.0, fill))
    else:
        cx = src_w * float(ss.get("safe_cx", 50)) / 100.0
        cy = src_h * float(ss.get("safe_cy", 50)) / 100.0
    if factor > 1.0:
        x0, y0, cw, ch = viewport.crop_rect(src_w, src_h, fit * factor, cx, cy,
                                            _SAFE_DISP_W, _SAFE_DISP_H)
        # crop 回的是 view(可能非連續),PNG 編碼前轉連續陣列。
        drawn = np.ascontiguousarray(imgio.crop(drawn, x0, y0, cw, ch))

    st.image(imgio.to_png_bytes(drawn), width=_STRETCH)
    st.caption(f"🛟 安全模式 · **{meta['name']}** · {meta['idx1']}/{meta['total']} · "
               f"{meta['w']}×{meta['h']} · {meta['bit']}-bit · {meta['channels']} · "
               f"{factor:.1f}× · 顯示 {len(kept)} 框")


# 🛟 安全模式的縮圖牆是原生 st.image + st.button(檔名/徽章要放得下),比元件版的純圖片窄條需要更寬的欄。
_left_w = 0.0001 if ss.thumb_collapsed else (1.7 if ss.get("safe_mode") else 0.85)
...
with left:
    if not ss.thumb_collapsed:
        ...
        # ── 縮圖牆分頁(2026-07-10)────────────────────────────────────────────────
        # 幾百張時整面牆一次算繪會鈍(安全模式 3N 個原生元素;元件模式 N 張 base64 進 args),
        # 分頁把 DOM / 首載 / 每次 rerun 的重建都封頂。兩種模式共用同一組控制。
        # ★ §4.l:keyed selectbox 與翻頁鈕都在「會 st.rerun() 的縮圖選取鈕」之前實例化。
        # 翻頁鈕只改 ss.tw_page、不呼叫 st.rerun() —— 按鈕在切片之前執行,本 run 下方就用新值。
        _tw_total = len(shown_items)
        _pp_opt = st.selectbox("每頁", [25, 50, 100, "All"], key="tw_per_page",
                               help="縮圖牆每頁張數;幾百張時建議 25–50,整牆一次算繪會鈍。")
        _pp = _tw_total if _pp_opt == "All" else int(_pp_opt)
        _pp = max(1, _pp)
        _n_pages = max(1, -(-_tw_total // _pp))
        # 換張(上一張/下一張/鍵盤/點縮圖)→ 頁自動跟著所選影像;手動翻頁瀏覽則不動 idx。
        if ss.get("_tw_follow_idx") != ss.idx or ss.get("_tw_last_pp") != _pp:
            ss._tw_follow_idx = ss.idx
            ss._tw_last_pp = _pp
            ss.tw_page = ss.idx // _pp
        ss.tw_page = max(0, min(ss.get("tw_page", 0), _n_pages - 1))
        if _n_pages > 1:
            _pgc = st.columns([1, 2, 1], vertical_alignment="center")
            if _pgc[0].button("◀", key="tw_pg_prev", width=_STRETCH, disabled=ss.tw_page <= 0):
                ss.tw_page -= 1
            if _pgc[2].button("▶", key="tw_pg_next", width=_STRETCH,
                              disabled=ss.tw_page >= _n_pages - 1):
                ss.tw_page += 1
            ss.tw_page = max(0, min(ss.tw_page, _n_pages - 1))
            _pgc[1].caption(f"第 {ss.tw_page + 1}/{_n_pages} 頁")
        _pg_start = ss.tw_page * _pp
        _pg_items = shown_items[_pg_start:_pg_start + _pp]
        if ss.get("safe_mode"):
            # 🛟 安全模式:不建 data-url items、不掛元件,直接原生算繪(見 _safe_thumbwall)。
            _safe_thumbwall(_pg_items, ss.idx, show_overlay, overlay_classes, _TW_CONF, _marks,
                            start=_pg_start)
        else:
            tw_items = []
            ...
            # 分頁後元件只看得到本頁:selected 傳頁內索引(所選不在本頁→ -1,main.js 對 -1 安全
            # =不高亮任何格);事件回來的 index 也是頁內 → 加 _pg_start 還原成全域。
            _sel_local = ss.idx - _pg_start
            if not (0 <= _sel_local < len(_pg_items)):
                _sel_local = -1
            ev_tw = thumbwall(tw_items, selected=_sel_local, height=620, key="thumbwall", markable=True)
            if ev_tw is not None:
                _tw_idx = _pg_start + ev_tw["index"]
                if ev_tw["type"] == "mark":
...
with center:
    if ss.compare_on:
        ...
        _meta_hud = {"name": cur["name"], "idx1": ss.idx + 1, "total": total,
                     "w": w, "h": h, "bit": bit, "channels": ch}
        if ss.get("safe_mode"):
            # 🛟 安全模式:server 端燒框 + 裁切縮放,不掛元件、不抓 /component/(見 _safe_viewer)。
            _safe_viewer(cur, kept, rois_draw, _meta_hud, focus_bbox)
        else:
...
st.markdown(
    ...
    f"data-cmp-view-mode='{_cmpp.get('view_mode', '')}' "
    f"data-dino-model='{_cmpp.get('dino_model', '')}' "
    f"data-dino-score='{_cmpp.get('dino_score', '')}' "
    f"data-dino-regions='{_cmpp.get('dino_regions', '')}'></div>",
    unsafe_allow_html=True,
)
